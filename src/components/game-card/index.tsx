    ...
    >
      ...
      >
        ...
        <CardContent className="min-h-16">
          ...
          <div id={`${data?.id}-card-tags`} className="flex gap-2 -ml-1">
            {hasAccoladesOrTags && (
              <div className="bg-black px-2 -mt-[2px] rounded-md">
                <p className="font-bold text-md">
                  {data?.accolade?.toUpperCase()}
                </p>
              </div>
            )}
          </div>
          {gameComingSoon && (
            <p className="leading-7 bottom-2 absolute text-lime-400 text-sm font-bold">
